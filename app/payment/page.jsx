...
const Payment = () => {
  ...
  const getAmount = async () => {
    try {
      ...
      // 🔥 Clean invalid combo codes
      const comboCodes = JSON.parse(localStorage.getItem("combo_event_codes") || "[]");
      const cartCodes = currentCart.map(i => i.event_code);

      const validComboCodes = comboCodes.filter(c => cartCodes.includes(c));

      if (validComboCodes.length === 0) {
        localStorage.removeItem("combo_event_codes");
      } else {
        localStorage.setItem("combo_event_codes", JSON.stringify(validComboCodes));
      }
