...
const Cart = () => {
  ...
  const hasNonComboEarlyBirdEvents = () => {
    ...
    // Find early bird events in cart that are NOT from combo
    return cart.some(item => 
      EARLY_BIRD_EVENTS.includes(item.event_code) && 
      !comboEventCodes.includes(item.event_code)
    );
  };

  const canApplyEarlyBird = hasNonComboEarlyBirdEvents();
  ...
  const calculateAmount = (currentCart, discountedCodes = []) => {
    ...
    if (comboItems.length > 0 && comboPrice > 0) {
  ...
  const getAmount = async (code, currentCart = cart) => {
    try {
      if (code) {
        ...
        console.log("Backend response with code:", res.data.data);
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Check if there's a saved promo code
      const savedCode = localStorage.getItem("early_code");
      
      if (savedCode) {
        // Restore the code to input field
        setEarlyCode(savedCode);
        // Apply the code to get discounted amount
        await getAmount(savedCode, currentCart);
      } else {
        await getAmount("", currentCart);
      }
    };
  ...
  const refreshCartAndAmount = async () => {
    ...
    const currentCart = res.data.data || [];
    setCart(currentCart);
    
    // Clean up combo codes for events no longer in cart
    const comboEventCodes = JSON.parse(localStorage.getItem("combo_event_codes") || "[]");
    const currentEventCodes = currentCart.map(item => item.event_code);
    const stillValidComboCodes = comboEventCodes.filter(code => currentEventCodes.includes(code));
    
    if (stillValidComboCodes.length === 0) {
      localStorage.removeItem("combo_event_codes");
    } else {
      localStorage.setItem("combo_event_codes", JSON.stringify(stillValidComboCodes));
    }
    
    // Check if promo code was previously applied
    const savedCode = localStorage.getItem("early_code");
    
    if (savedCode) {
      // Reapply the code after cart changes
      await getAmount(savedCode, currentCart);
    } else {
      await getAmount("", currentCart);
    }
